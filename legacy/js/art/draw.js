 ...
 *
 * Seven generators — elevation, massing, court, section, plan, screen,
 * contour — each producing an SVG string from a string seed. No raster assets
 * ship with this site; every image you see is drawn here at runtime.
 *
 ...
 * from `palette.js`, seeded identically to the geometry.
 *
...
import { rng, n } from './rng.js';
import { palette, mix, shade } from './palette.js';

const NS = 'http://www.w3.org/2000/svg';
const W = 1000;
const uid = (s) => `k${(s || 'x').replace(/[^a-z0-9]/gi, '').slice(0, 10)}`;

/* --------------------------------------------------------------------------
   shared pieces
   -------------------------------------------------------------------------- */

function svg(h, defs, body) {
...
const brickDef = (id, p, course, len) => `
...
function tree(r, p, x, base, h) {
  const w = h * r.float(0.55, 0.85);
  const c1 = p.leaf, c2 = shade(p.leaf, 0.16), c3 = shade(p.leaf, -0.16);
  return `<g>
...
export function elevation(seed, ratio = 0.75) {
  ...
  const defs = skyDef(id, p) + glassDef(id, p) +
    brickDef(id, p, course, bayW[0] / r.int(4, 7));
...
export function massing(seed, ratio = 0.75) {
  ...
  const iso = (x, y, z) => [cx + (x - y) * u, cy + (x + y) * u * k - z * u];
  ...
  for (let i = -g; i <= g; i += 2) {
    const a = iso(i, -g, 0).map(n), b = iso(i, g, 0).map(n);
    const c = iso(-g, i, 0).map(n), d = iso(g, i, 0).map(n);
  ...
  const box = (x, y, z, w, d, h) => {
    const pt = (X, Y, Z) => iso(X, Y, Z).map(n).join(' ');
    ...
    let win = '';
    const cols = Math.max(1, w), rows = Math.max(1, h);
  ...
  for (let i = 0; i < r.int(3, 6); i++) {
    const w = r.int(2, 4), d = r.int(2, 4);
    const x = r.int(-4, 2), y = r.int(-4, 2);
...
export function court(seed, ratio = 0.75) {
...
export function section(seed, ratio = 0.75) {
  ...
  const inner = (l) => (setback && l >= setback ? [x0 + setIn, x1 - setIn] : [x0, x1]);
  ...
  const cut = (l) => voidKind !== 'none' && l > vFrom && l <= vTo;
  ...
  for (let l = 1; l <= levels; l++) {
    const y = ground - fh * l;
    const [a, b] = inner(l);
    const spans = cut2(l) ? [[a, Math.min(vA, b)], [Math.max(vB, a), b]] : [[a, b]];
  ...
  if (r.chance(0.78) && levels > 1) {
    ...
    const sx = r.chance(0.5)
      ? x0 + span * r.float(0.06, 0.2)
      : x1 - sw - span * r.float(0.06, 0.2);
    for (let l = 0; l < levels; l++) {
...
export function plan(seed, ratio = 0.75) {
  ...
  const x0 = m, y0 = H * r.float(0.08, 0.14);
  const x1 = W - m, y1 = H - H * r.float(0.08, 0.14);
  ...
  if (r.chance(0.72)) {
    ...
    const pw2 = cw * 0.52, ph = ch * 0.34;
    s += `<rect x="${n(cx + (cw - pw2) / 2)}" y="${n(cy + (ch - ph) / 2)}"
            width="${n(pw2)}" height="${n(ph)}" fill="url(#${id}-sky)"/>`;
    const tx = cx + cw * r.float(0.16, 0.84), ty = cy + ch * r.float(0.16, 0.84);
  ...
  // columns
  const cols = r.int(3, 5), rows = r.int(2, 4);
  for (let i = 1; i < cols; i++) {
    for (let j = 1; j < rows; j++) {
      const px = x0 + ((x1 - x0) / cols) * i;
      const py = y0 + ((y1 - y0) / rows) * j;
      if (court2 && px > court2.cx && px < court2.cx + court2.cw &&
          py > court2.cy && py < court2.cy + court2.ch) continue;
...
export function screen(seed, ratio = 1.4) {
...
export function contour(seed, ratio = 0.75) {
  ...
  for (let i = 0; i < bands; i++) {
    const a = line(i);
    const b = i + 1 < bands ? line(i + 1) : a.map(([x]) => [x, H + 40]);
    const d = `M${a.map(([x, y]) => `${n(x)} ${n(y)}`).join('L')}
               L${b.slice().reverse().map(([x, y]) => `${n(x)} ${n(y)}`).join('L')}Z`;
  ...
  // the building
  const fw = W * r.float(0.18, 0.3), fh2 = H * r.float(0.14, 0.24);
  const fx = W * r.float(0.2, 0.56), fy = H * r.float(0.3, 0.56);
  ...
  for (let i = 0; i < r.int(5, 10); i++) {
    const px = W * r.float(0.04, 0.96), py = H * r.float(0.06, 0.96);
...
export function portrait(seed, ratio = 1) {
  ...
  const bg = r.pick([
    mix(p.mat.wall, '#e6dccb', 0.55), mix(p.leaf, '#dfe3d2', 0.6),
    mix(p.sky.low, '#ffffff', 0.3), mix(p.mat.lite, '#ffffff', 0.35),
...
const KINDS = { elevation, massing, court, section, plan, screen, contour, portrait };
export const KIND_NAMES = ['elevation', 'massing', 'court', 'section', 'plan',
  'screen', 'contour'];
 ...
 */
export function kindFor(seed, types = []) {
  const r = rng(seed + ':kind');
  const t = types.join(' ').toLowerCase();
  if (/urban|complex|industrial/.test(t)) return r.weighted(
    ['massing', 'elevation', 'contour', 'court', 'plan'], [5, 4, 3, 2, 1]);
  if (/interior|renovation/.test(t)) return r.weighted(
    ['section', 'elevation', 'court', 'plan', 'screen'], [4, 4, 2, 2, 2]);
  if (/hospitality|office|commercial/.test(t)) return r.weighted(
    ['elevation', 'massing', 'section', 'screen', 'court'], [6, 3, 3, 2, 2]);
  if (/villa/.test(t)) return r.weighted(
    ['elevation', 'court', 'contour', 'massing', 'section'], [4, 4, 3, 3, 2]);
  return r.weighted(
    ['elevation', 'massing', 'section', 'court', 'screen', 'plan', 'contour'],
    [7, 4, 3, 3, 2, 2, 1]);
}

export function draw(kind, seed, ratio) {
  return (KINDS[kind] || elevation)(seed, ratio);
}

/** The three-drawing set shown on a project detail page. */
export function drawingSet(seed, types = []) {
