 ...
 */
/*
 * PROJECT DECISION: there is NO production error tracker wired into this app, so those
 * three call sites currently report nowhere and this module stays dev-only.
 * SYMPTOM that made this the choice: none — it is a scope call. Provisioning and paying
 *   for a tracker was not in the two-day delivery window, and a half-wired reporter that
 *   drops events is worse than a known gap.
 * RE-ENABLE WHEN: a tracker exists for this project. Add its DSN to `server-env.ts`, then
 *   call it from the three places listed above — nothing in THIS module changes, because
 *   dev logging and production reporting are separate concerns and must stay so.
 */
