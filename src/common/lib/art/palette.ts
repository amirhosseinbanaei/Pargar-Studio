 ...
 *
 * Ported from `legacy/js/art/palette.js`. These hex values are DRAWING
 * pigment, not interface colour — they are the ink of a generated image, the
 * way a photograph's pixels are, and they are deliberately not design tokens.
 * The site's chrome tokens live in `src/app/globals.css` and none of them
 * appear here. PURE: no DOM, safe on the server.
 */
import { rng } from './rng';

/** One facade material, as actually built in north Tehran. */
...
/** The full scheme a generator draws with. */
export interface Palette {
  ...
  glass: string;
  glassLite: string;
  /** a lit room seen from outside */
  warm: string;
  /** the shadow side of a mass */
  shade: string;
  ground: string;
  ink: string;
}
...
const LEAVES: readonly string[] = ['#4d6b3a', '#5f8347', '#3d5730', '#6d8f4e', '#557640'];

/** Build a coherent scheme. */
export function palette(seed: string, opts: { night?: boolean } = {}): Palette {
...
export const shade = (c: string, t: number): string =>
  t >= 0 ? mix(c, '#ffffff', t) : mix(c, '#000000', -t);
