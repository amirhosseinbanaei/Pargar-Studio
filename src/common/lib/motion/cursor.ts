// src/common/lib/motion/cursor.ts
/**
 * The custom cursor.
 *
 ...
 *
 * Ported from `legacy/js/ui/cursor.js`. No 'use client' here — see anim.ts.
 */
import { onTick, spring, damp, round, reduced } from './anim';

const MAGNET_SEL = '.magnet, a[href], button:not([disabled]), .card, .row, .fopt, .col__hit';

/** What `initCursor` hands back. `refresh` is called after any layout change. */
...
export function initCursor(): CursorHandle {
  ...
  if (!ring || !dot || !label) return { destroy() {} };

  document.documentElement.classList.add('has-cursor');

  let px = innerWidth / 2;
  let py = innerHeight / 2; // raw pointer
  let live = false;
  ...
  let target: HTMLElement | null = null; // currently magnetised element
  ...
  const onOver = (e: PointerEvent): void => {
    const el = e.target instanceof Element ? e.target.closest<HTMLElement>(MAGNET_SEL) : null;
    if (el === target) return;
    target = el;
  ...
  const onDown = (): void => {
    rs.target = target ? 1.2 : 0.7;
  };
  const onUp = (): void => {
    rs.target = target ? 1.55 : 1;
  };
  const onLeave = (): void => {
    live = false;
  ...
  const stop = onTick(dt => {
    ...
    ring.style.transform = `translate3d(${round(rx.value)}px, ${round(ry.value)}px, 0) scale(${round(rs.value, 1000)})`;
