 ...
 *
 * Ported from `legacy/js/core/anim.js`. This file needs the DOM but carries NO
 * 'use client' directive, deliberately: the directive belongs on the single
 * React component that mounts these modules, not on a shared library. A
 * directive here would drag every module that so much as imports a type from
 ...
 */
export const instant = (): boolean => reduced() || document.hidden;
...
export const round = (v: number, p = 1000): number => Math.round(v * p) / p;

/* --------------------------------------------------------------------------
   the shared ticker
   -------------------------------------------------------------------------- */

/** A per-frame subscriber. `dt` is seconds since the last frame. */
 ...
 *
 * Guarded on `document` because this module is imported (for its types and its
 * pure maths) from files that a Server Component may reach. The listener is
 * registered once per module instance, exactly as it was on the static site.
 */
...
/**
 * A WAAPI `Animation` plus `done`.
 *
 * `done` exists because `finished` cannot be trusted — see `animate()` below.
 * Everything in this codebase awaits `done`.
 */
export interface TrackedAnimation extends Animation {
  done: Promise<void>;
}

export interface AnimateOptions extends Omit<KeyframeAnimationOptions, 'fill'> {
  fill?: FillMode;
  /** Apply `will-change` for the duration and remove it after. Default true. */
  hint?: boolean;
}
...
export function animate(
  ...
  const flat = instant();
  const d = flat ? 1 : (duration as number);
  const dl = flat ? 0 : (delay as number);

  if (hint && el instanceof HTMLElement) {
  ...
  anim.done = (
    flat
      ...
      : Promise.race([
          ...
          new Promise<void>(res => setTimeout(res, d + dl + 150)),
        ])
  ).then(() => {
    if (hint && el instanceof HTMLElement) el.style.willChange = '';
  });

  return anim;
}

const cssName = (k: string): string => k.replace(/[A-Z]/g, m => '-' + m.toLowerCase());

export interface StaggerOptions extends AnimateOptions {
  /** ms between neighbours */
  step?: number;
  /** index the stagger radiates from */
  from?: number;
}

/** Animate a list with a stagger. Resolves when the last one lands. */
export function stagger(
  els: Iterable<Element | null | undefined>,
  keyframes: Keyframe[] | PropertyIndexedKeyframes,
  opts: StaggerOptions = {},
): Promise<unknown> {
  ...
  const anims = list
    .map((el, i) =>
      animate(el, keyframes, {
        ...rest,
        delay: ((rest.delay as number) || 0) + (i - from) * step,
      }),
    )
    .filter((a): a is TrackedAnimation => Boolean(a));
...
export interface FlipOptions {
  ...
  /** ms between neighbours */
  step?: number;
  /** also invert size, not only position */
  scale?: boolean;
  delay?: number;
}
...
export function flip(
  ...
  const { duration = 900, easing = EASE.expo, step = 0, scale = false, delay = 0 } = opts;

  const list = [...els].filter((el): el is Element => Boolean(el));
  const first = list.map(el => el.getBoundingClientRect());

  mutate();

  const last = list.map(el => el.getBoundingClientRect());
  const anims: TrackedAnimation[] = [];
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const from =
      `translate3d(${round(dx)}px, ${round(dy)}px, 0)` +
      (scale ? ` scale(${round(sx, 10000)}, ${round(sy, 10000)})` : '');

    const anim = animate(
      list[i],
      [{ transform: from }, { transform: 'translate3d(0, 0, 0)' + (scale ? ' scale(1, 1)' : '') }],
